// 게시글 목록 화면에서 사용하는 뷰 모델 클래시 이다. + 상태 관리
...
// AutoDisposeAsyncNotifier 를 사용해야 하는 이유를 알자!! AutoDisposeAsyncNotifier = 자동으로 메모리를 해제
class PostListViewModel extends AutoDisposeAsyncNotifier<PostList?> {
  // 상태관리가 아닌 멤버 변수
  ...
  PostList? build() {
    ...
    // 콜백 메서드 등록
    ref.onDispose(() {
      // PostListViewModel 메모리에서 내려갈 때 콜백 호출...
  ...
  // 5. refreshController.loadCompleted() (동그라미 제거)
  Future<void> init() async {
    try {
      Map<String, dynamic> resBody = await postRepository.findAll();
      if (!resBody['success']) {
        // 통신은 성공이지만 내부적으로 오류로 본다 --> 적절한 에러 메세지를 던져 줌
        ExceptionHandler.handleException(
            resBody['errorMessage'], StackTrace.current);
        return; // 실행의 제어권 반납
      }
      // 통신 성공 -->
      state = PostList.fromMap(resBody) as AsyncValue<PostList?>;
    } catch (e, stackTrace) {}
  }
}
